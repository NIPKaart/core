    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6 px-4 py-6 sm:px-6">
                <h1 className="mb-2 text-2xl font-bold">My favorite locations</h1>
                <p className="text-muted-foreground">All your favorite parking locations and garages at a glance.</p>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <Input
                        className="w-full sm:max-w-sm"
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        placeholder="Search favorites by name, city, or municipality..."
                        autoFocus
                    />
                </div>
                ...
                {filtered.length === 0 ? (
                    <div className="mt-16 flex flex-col items-center justify-center gap-4 text-center">
                        ...
                        <span className="text-lg font-semibold text-muted-foreground">You haven&apos;t added any favorites yet.</span>
                        <span className="text-sm text-muted-foreground">Browse the map and click the heart icon to favorite locations.</span>
                                ...
                                >
                                    <CardHeader className="flex flex-row items-center gap-3 pb-2">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <CardDescription className="mt-0.5 text-xs capitalize">{fav.type.toLowerCase()}</CardDescription>
                                        </div>
                                    </CardHeader>
                                    <CardFooter className="mt-2 flex items-center justify-between border-t pt-4">
                                        <div className="flex flex-col text-xs text-zinc-500">
                                            {fav.country && <span>Country: {fav.country}</span>}
                                        </div>
