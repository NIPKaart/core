...
| Frontend candidates | Follow-up; retained. The welcome page (currently used by the feedback test), header layout/header, simple/card auth layouts, appearance dropdown, banner, duplicate `ui/icon`, Sonner wrapper and toggle/toggle-group have no reachable production consumer in the import/page scan. Removal is deferred to respect the narrower cleanup scope. Active sidebar/split-auth/public/map layouts and responsive hooks remain. |
| Dependency candidates | Follow-up; retained. `http-interop/http-factory-guzzle` and `laravel/sail` have only root Composer dependency edges, with no discovered PSR-17 factory or Sail setup consumer. `next-themes` and Radix toggle/toggle-group support the retained unused wrappers. No manifests, lockfiles or dependency versions change in this pass. |
...
| Operator commands | Keep / follow-up. Artisan auto-discovers `nipkaart:make-admin`, `user:create`, `logs:clear` and `app:inject-version`. The documented/tested admin command does not replace `user:create`'s non-admin roles and verification choices. Review that older command's validation/verification separately. Log clearing and writing `.version` retain operational behavior. |
