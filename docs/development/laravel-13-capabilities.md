...
**NIPKaart problem and existing solution.** A future mobile/public client may benefit from standardized resource identity, relationships and pagination. Today, [SpacesInfoController](../../app/Http/Controllers/Frontend/SpacesInfoController.php) returns explicit JSON for the browser, [DestinationSearchController](../../app/Http/Controllers/Frontend/DestinationSearchController.php) returns destination suggestions and resolution results, and [FavoriteResource](../../app/Http/Resources/FavoriteResource.php) is a conventional `JsonResource` with NIPKaart-specific availability semantics. Browser endpoints use throttling and the web/session middleware; details include user-specific favorite/confirmation fields when signed in.
