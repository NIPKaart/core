...
Historical #1180 validation includes fresh DDEV/PHP 8.4/PostgreSQL 18.4/PostGIS 3.6 startup, all 15 migrations, baseline/sample seeders, SQL-coordinate derivation, radius/distance/viewport behavior, mixed-source visibility, real GiST query-plan use, native backup/restore and real Scout rebuilds. CI runs the existing test suite against PostgreSQL. The then-new spatial, relationship, notification and Scout regression tests were removed at the owner's request; those earlier successful runs remain implementation evidence. Under #1199, focused permanent spatial coverage is restored in `ParkingLocationTest`, `ParkingDiscoveryTest`, `GeoPointTest` and `GeoBoundsTest`; see [quality checks](quality-checks.md) for the full-suite and TIA workflows. The backup/restore and planner rehearsals remain historical evidence. Remote CI and review status are reported on the PR.
