...
Dataset `nl-eindhoven` and selection `gehandicapten-all` use the same delivery envelope. Run `php artisan nipkaart:register-eindhoven <municipality-id>` for an existing Eindhoven municipality in `NL-NB`, country `NL`. The source is the [municipal parkeerplaatsen dataset](https://data.eindhoven.nl/explore/dataset/parkeerplaatsen/), filtered by `type_en_merk = 'Parkeerplaats Gehandicapten'`. The collector's source adapter uses the existing HTTP client to preserve completeness and original IDs not exposed by the installed Eindhoven package.
