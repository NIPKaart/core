...
| Collector | `disabled-parking`, its existing Python >=3.12 environment and reusable source package. Check source package versions when a required fix is made. |
...
| Scheduled execution | One persistent Docker container per collector repository on an owner-managed Linux host or cloud VM. An internal scheduler runs finite commands serially; no container per municipality. Core uses its own scheduler/queue. Implementation in #775/#1217. |
